...
# ---------------------------------------------------------
# Load .env from the same folder as this app.py file.
...
def login_required(view_func):
    """Redirect to Google sign-in when the user is not logged in."""
...
@app.route("/login")
def login():
    # Check the values loaded when the application started.
    if not GOOGLE_CLIENT_ID or not GOOGLE_CLIENT_SECRET:
        abort(
            503,
            description=(
                "Google Sign-In isn't configured on this server yet. "
                "Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to the "
...
def auth_callback():
    try:
        ...
        if not user_info:
            # Some OAuth responses may require userinfo explicitly.
...
def resolve_font(
    ...
    """
    Try to reuse the PDF's embedded font.
    Falls back to a standard PDF font.
    """
    try:
        page = doc[page_num]
    ...
    if not f or f.filename == "":
        abort(
            ...
            description=(
                ...
                "Please go back and select a PDF."
            ),
        )
    ...
    if not files or all(
        ...
        abort(
            ...
            description=(
                ...
                "Please go back and select at least one photo."
            ),
        )
    ...
    if processed == 0:
        abort(
            ...
            description=(
                ...
                "Please try JPG, PNG, or WEBP images."
            ),
        )
...
def extract_pdf_text():
    ...
    if not file or file.filename == "":
        abort(
            ...
            description=(
                "No PDF was received. "
                "Please go back and choose a file."
            ),
        )
    ...
    if not blocks:
        abort(
            400,
            description=(
                "No editable text was found in this PDF "
...
def apply_pdf_edits():
    ...
    try:
        ...
        abort(
            400,
            description=(
                "The original PDF data was lost "
    ...
    try:
        for page_num, edits in edits_by_page.items():
            ...
            # Redact original text areas first.
            for e in edits:
                rect = fitz.Rect(
                    e["x0"],
                    e["y0"],
            ...
            for e in edits:
                ...
                fontname, fontfile = resolve_font(
                    doc,
                    page_num,
                    e["font"],
                    tmp_font_files,
...
def service_unavailable(e):
    return (
        e.description
        or "This feature is temporarily unavailable."
    ), 503
